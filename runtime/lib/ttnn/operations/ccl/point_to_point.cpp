...
void run(const ::tt::target::ttnn::PointToPointOp *op,
  ...
  auto calcIdFromCoords =
      [&](const flatbuffers::Vector<int64_t> *coords) -> size_t {
  ...
  size_t sendId = calcIdFromCoords(op->send_coord()->coords());
  size_t recvId = calcIdFromCoords(op->receive_coord()->coords());
