...
#include "operations/data_movement/copy_host_to_device_tensor.h"
...
void ProgramExecutor::runOperation(const ::tt::target::ttnn::Operation *op) {
  ...
  case ::tt::target::ttnn::OpType::CopyHostToDeviceTensorOp: {
    return operations::data_movement::run(
        op->type_as_CopyHostToDeviceTensorOp(), getContext());
  }
