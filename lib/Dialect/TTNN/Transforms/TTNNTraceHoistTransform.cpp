...
static ::mlir::RankedTensorType getTraceIdType(MLIRContext *ctx) {
  ...
  BufferType bufferType = BufferType::SystemMemory;
  ...
  TTNNLayoutAttr layoutAttr = TTNNLayoutAttr::get(
      ctx, /*shape=*/{},
      ::mlir::IntegerType::get(ctx, 32, IntegerType::Unsigned), bufferType,
      ttcore::GridAttr::get(ctx), memoryLayoutAttr, tensorMeshShardingAttr,
      collapseDimsRef);

  return ::mlir::RankedTensorType::get(
      /*shape=*/{}, ::mlir::IntegerType::get(ctx, 32, IntegerType::Unsigned),
      layoutAttr);
}

static GetDeviceOp getOrInsertDevice(RewriterBase &rewriter, Block *block) {
  ...
  auto deviceOp = rewriter.create<ttnn::GetDeviceOp>(
      parentOp->getLoc(), rewriter.getType<DeviceType>(),
      ttnn::MeshShapeAttr::get(parentOp->getContext(), meshShape[0],
                               meshShape[1]),
      ttnn::MeshOffsetAttr::get(parentOp->getContext(), meshOffset[0],
                                meshOffset[1]));
...
private:
  ...
  TraceSmallString getTraceFuncName(func::FuncOp funcOp,
    ...
    TraceSmallString traceFuncName(funcOp.getName());
  ...
  TraceSmallString getCaptureTraceFuncName(func::FuncOp funcOp,
    ...
    runAndCaptureTraceFuncName.append("run_");
    runAndCaptureTraceFuncName.append(getTraceFuncName(funcOp, traceFuncIndex));
    runAndCaptureTraceFuncName.append("_and_capture_trace");
  ...
  TraceSmallString getExecuteTraceFuncName(func::FuncOp funcOp,
    ...
    executeTraceFuncName.append("_trace");
  ...
  createTraceFunction(func::FuncOp funcOp,
    ...
    for (size_t i = 0; i < inputs.size(); i++) {
      valueMap.insert({inputs[i], traceFuncOp.getArguments()[i]});
    }
  ...
  createRunAndCaptureTraceFunction(func::FuncOp funcOp,
    ...
    auto deviceOp =
        getOrInsertDevice(rewriter, runAndCaptureTraceFuncEntryBlock);
    ...
    for (size_t i = 0; i < inputSlots.size(); i++) {
      mlir::Value input = runAndCaptureTraceFunc.getArguments()[i];
      ...
      builder.create<ttnn::CopyHostToDeviceTensorOp>(
          runAndCaptureTraceFunc.getLoc(), fromDeviceOp, inputSlots[i],
  ...
  mlir::LogicalResult createExecuteTraceFunction(func::FuncOp funcOp,
    ...
    auto deviceOp = getOrInsertDevice(rewriter, executeTraceFuncEntryBlock);
    mlir::Value traceId = executeTraceFunc.getArguments()[0];
